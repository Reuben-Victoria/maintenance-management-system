...
export default function AdminCategories() {
  ...
  const createMutation = useCreateCategory({
    mutation: {
      onSuccess: () => { toast({ title: "Category added" }); setNewCatName(""); setNewCatDesc(""); refetch(); },
      onError: (err) => toast({ title: "Failed to add", description: err.data?.error, variant: "destructive" })
    }
  });

  const updateMutation = useUpdateCategory({
    mutation: {
      onSuccess: () => { toast({ title: "Category updated" }); setEditingId(null); refetch(); },
      onError: (err) => toast({ title: "Failed to update", description: err.data?.error, variant: "destructive" })
    }
  });

  const deleteMutation = useDeleteCategory({
    mutation: {
      onSuccess: () => { toast({ title: "Category deleted" }); refetch(); },
      onError: (err) => toast({ title: "Failed to delete", description: err.data?.error, variant: "destructive" })
    }
  });
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 max-w-4xl mx-auto">
      <div>
        ...
        <p className="text-muted-foreground">Define the types of maintenance requests users can submit.</p>
      </div>
      ...
      <Card className="border-border/50 shadow-sm">
        ...
        <CardContent>
          <form onSubmit={handleCreate} className="flex flex-col sm:flex-row gap-4 items-end">
            ...
            <Button type="submit" disabled={!newCatName || createMutation.isPending} className="w-full sm:w-auto shrink-0">
                ...
                <TableRow key={cat.id}>
                  {editingId === cat.id ? (
                    <>
                      ...
                      <TableCell><Input value={editDesc} onChange={e => setEditDesc(e.target.value)} className="h-8" /></TableCell>
                      <TableCell className="text-muted-foreground text-sm">{formatDate(cat.createdAt)}</TableCell>
                      <TableCell className="text-right space-x-2">
                        <Button size="sm" onClick={() => handleUpdate(cat.id)} disabled={updateMutation.isPending}>Save</Button>
                    ...
                    <>
                      ...
                      <TableCell className="text-right space-x-1">
                        <Button size="icon" variant="ghost" onClick={() => startEdit(cat)}><Edit2 className="h-4 w-4" /></Button>
                        <Button 
                          size="icon" 
                          variant="ghost" 
